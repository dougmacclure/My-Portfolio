...
//otherwise, current node is node we want to delete
		...
		//we'll need this later in case 2 and 3, so to save time, declare it here
...
//we want to find bnode's predecessor, to do so we do a simple inorder traversal recursively
	                {
			//have our temporary pointer pointed towards bnode's left child, which is
			//logically less than current node
        
	                temp = bnode->left;
			
			//initialize our lag pointer to a NULL to indicate if bnode->left is
			//bnode's successor or not

                        binTreeNode<T> *lag = NULL;
 
			//go to right side of bnode's left side, and keep going until 
			//the right side of our temporary pointer is NULL

                        while(temp->right!=NULL)
                        
				{
                        
				//lag is now no longer NULL, as we have satisfied condition
				//at least once, and also have lag be one step behind temp
			        lag = temp;
                                temp = temp->right;
                        
				}
			//exchange data values of "current" node with predecessor
			...
			//otherwise, we didn't iterate through while loop, so
                                ...
                                remove(bnode->left, bnode->left->data);
                }
        }
}
